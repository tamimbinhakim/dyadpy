...
import { createTytheResources } from "../src/index.js";
...
describe("query resource", () => {
  it("unwraps Result.ok onto resource data", async () => {
    ...
    await createRoot(async (dispose) => {
      const resources = createTytheResources({ getIssue } as unknown as TestApi);
...
describe("mutation resource", () => {
  it("calls mutate and tracks state", async () => {
    ...
    await createRoot(async (dispose) => {
      const resources = createTytheResources({ createIssue } as unknown as TestApi);
