...
import { createTytheStores } from "../src/index.js";
...
describe("query store", () => {
  it("unwraps Result.ok onto .data", async () => {
    ...
    const stores = createTytheStores({ getIssue } as unknown as TestApi);
...
describe("mutation store", () => {
  it("calls mutate and exposes data", async () => {
    ...
    const stores = createTytheStores({ createIssue } as unknown as TestApi);
...
describe("subscription store", () => {
  it("forwards stream events to onEvent and closes cleanly", async () => {
    ...
    const seen: number[] = [];
    const stores = createTytheStores({ events } as unknown as TestApi);
